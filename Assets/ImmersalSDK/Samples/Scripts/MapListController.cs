...
namespace Immersal.Samples
{
    ...
    public class MapListController : MonoBehaviour
    {
        ...
        public async void OnValueChanged(TMP_Dropdown dropdown)
        {
            ...
            {
                Debug.Log(" Embedded map selected, loading..."); 
            ...
            else
            {
                if (value >= 0)
                {
                    ...
                    Debug.Log($" Dropdown map selected: {map.name} (id: {map.id})"); 
                    LoadMap(map);
                }
            }
        }
        ...
        {
            Debug.Log($" LoadMap() called for job id: {job.id}"); 

            JobLoadMapBinaryAsync j = new JobLoadMapBinaryAsync();
            j.id = job.id;
            j.OnResult += async (SDKMapResult result) =>
            {
                Debug.LogFormat(" Map data received: {0} ({1} bytes)", job.id, result.mapData.Length); 
            ...
            j.OnError += (e) =>
            {
                Debug.LogError($" Error loading map {job.id}: {e}"); 
            };
